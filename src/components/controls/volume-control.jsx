...
const mapDispatchToProps = (dispatch) => ({
  onChange: (type) => (value) => dispatch(afSetVolume(volumePathFor(type), value)),
  ...
  <div style={{display:'flex'}}>
    {
      noteDisplayOrder.map((type, idx) => (
        <div key={`volumeControl${idx}`} style={{margin: '5px'}}>
          <div style={{lineHeight: '12pt', fontFamily: 'Bravura', display: 'flex', justifyContent: 'center', color: (mute(type) ? 'red' : undefined)}}>
            {type}
          </div>
